...
local function validate_parent(current_row, target_row, level)
	...
	local _, current_parent_start, current_parent_end = heading.get_parent_section_bounds(current_row, level)
	local _, target_parent_start, target_parent_end = heading.get_parent_section_bounds(target_row, level)
...
local function validate_containment(current_start, current_end, target_start, target_end, parent)
	if parent and parent.row ~= nil then
		return current_start >= parent.start
			and current_end <= parent.finish
			and target_start >= parent.start
			and target_end <= parent.finish
	else
		local global_start, global_end = 1, vim.api.nvim_buf_line_count(0)
		return current_start >= global_start
			and current_end <= global_end
			and target_start >= global_start
			and target_end <= global_end
	end
end
...
local function validate_siblings(current_row, target_row, level, parent)
	...
	local sibling_headers =
		sibling_utils.list_sibling_headers_of_level_in_bounds(level, parent.row, start_row, parent.finish)

	local current_idx, target_idx
	for i, v in ipairs(sibling_headers) do
		if v == current_row then
			current_idx = i
		end
		if v == target_row then
			target_idx = i
		end
	end
...
function M.get_validated_swap_context(current_row, target_row)
	...
	if
		not validate_containment(
			section_info.current_start,
			section_info.current_end,
			section_info.target_start,
			section_info.target_end,
			parent_info
		)
	then
		return nil
